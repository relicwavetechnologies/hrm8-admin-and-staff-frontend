...
export default function Consultant360NotificationsPage() {
  const {
    notifications,
    unreadCount,
    isLoading,
  ...
  const alerts = notifications.filter(
    (n) =>
      ...
      n.type?.includes('LEAD_CONVERSION')
  );
  ...
  const getNotificationIcon = (type: string) => {
    ...
    if (type?.includes('COMMISSION') || type?.includes('LEAD')) {
  ...
  const getNotificationColor = (type: string, read: boolean) => {
    ...
    if (type?.includes('REFUND') || type?.includes('WITHDRAWAL_APPROVED') || type?.includes('COMMISSION_EARNED') || type?.includes('LEAD_CONVERTED')) {
          ...
          <div
            className={cn(
              ...
              item.read
                ...
                : item.type?.includes('COMMISSION_EARNED') || item.type?.includes('LEAD_CONVERTED')
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">
                Stay informed with jobs, leads, earnings, and updates
              </p>
            </div>
          </div>
        </div>
